...
def check_game_over():
    ...
    for symbol in [player1, player2]:
        ...
        if board[0][2] == board[1][1] == board[2][0] == symbol:
            ...
            game_over = True
    # Check for a tie
    if all([square != '' for row in board for square in row]) and winner is None:
        game_over = True
...
while True:
    ...
    if game_over:
        ...
        text = font.render(f"Turn: {turn} - Player {current_player}'s turn", True, BLACK)
        text_rect = text.get_rect(center=(SCREEN_WIDTH/2, SCREEN_HEIGHT/2))
        screen.blit(text, text_rect)
